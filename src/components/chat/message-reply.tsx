...
import { Camera, X, FileText, Video, Volume2 } from "lucide-react";
...
const MessageReply = () => {
  ...
  const generateReplyContent = () => {
    switch (selectedMessage?.message_type) {
      ...
      case "audio":
        return (
          <div className="flex items-center gap-1">
            <Volume2 className="object-contain size-4" />
  ...
  return (
    <div
      className={cn(
        "absolute inset-x-0 z-20 -top-[75px] px-4 pt-4 mb-0 bg-white",
        {
          "-top-[113px]": selectedMessage?.message_type === "image",
        }
      )}
    >
