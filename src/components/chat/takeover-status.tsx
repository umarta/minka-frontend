...
export function TakeoverStatus({
  ...
  const isInTakeover =
    contact.is_takeover_by_admin || takeoverStatus?.is_takeover;
  ...
  if (!isInTakeover) {
    ...
    tooltipText = isCurrentUserTakeover
      ? "Anda takeover"
      : `Ditakeover oleh ${badgeText}`;
  }
          ...
          {!isInTakeover ? (
            <div>
              ...
              <p className="mb-3 text-sm text-gray-600">
                Pesan sedang dijawab otomatis oleh AI. Klik Take Over untuk
                handle manual.
              </p>
              <Button
                onClick={handleSetTakeover}
                disabled={isLoading}
                size="sm"
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                >
                  {isCurrentUserTakeover ? "You" : badgeText}
                </Badge>
                <span className="text-sm text-gray-600">
                  sedang handle chat ini
                </span>
              </div>
              {takeoverStatus?.takeover_at && (
                <div className="flex items-center gap-1 mb-2 text-xs text-gray-500">
                  <Clock className="w-3 h-3" />
                  {new Date(takeoverStatus.takeover_at).toLocaleTimeString()}
                </div>
              )}
              {takeoverStatus?.expires_at && (
                <Alert className="mb-2 border-orange-200 bg-orange-50">
                  ...
                  <AlertDescription className="text-orange-800">
                    Takeover expires at{" "}
                    {new Date(takeoverStatus.expires_at).toLocaleTimeString()}
                  </AlertDescription>
                </Alert>
              )}
              {isCurrentUserTakeover ? (
                ...
                <p className="text-sm text-gray-600">
                  Admin lain sedang handle chat ini. Anda bisa takeover jika
                  sudah dilepas.
                </p>
              )}
            </div>
