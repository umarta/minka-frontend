  ...
  subscribeWithSelector((set, get) => ({
    ...
    setTakeover: async (contactId, adminId) => {
      try {
        ...
        set((state) => ({
          contacts: state.contacts.map((contact) =>
            contact.id === contactId
              ? {
                  ...contact,
                  is_takeover_by_admin: true,
                  takeover_by_admin_id: adminId,
                }
              : contact
          ),
        }));
        ...
        set({
          error:
            error instanceof Error ? error.message : "Failed to set takeover",
        });
        throw error;
      }
    },
        ...
        set((state) => ({
          contacts: state.contacts.map((contact) =>
            contact.id === contactId
              ? {
                  ...contact,
                  is_takeover_by_admin: false,
                  takeover_by_admin_id: undefined,
                }
              : contact
          ),
        }));
...
export const useContactFilters = () =>
  useContactStore((state) => state.filters);
